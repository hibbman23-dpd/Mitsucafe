...
### 🚫 TUYỆT ĐỐI KHÔNG chạy `unittest discover` trên máy này

Đã cắn thật hai lần, lần gần nhất ngày 2026-08-05 đẩy **47 job rác** vào hàng đợi máy in của quán.

Cơ chế: `test_routes_spool.py` **tự đặt `PRINT_ENGINE=legacy` ngay lúc import**, đè biến môi trường truyền từ ngoài vào; rồi `test_routes.py` không mock `_print_receipt_bytes` nên gọi thẳng `lpr` tới máy in thật. Ép `GATEWAY_DB` sang file tạm **không** chặn được đường này — không biến môi trường nào bên ngoài chặn được.

Chỉ chạy đích danh 4 file test chấm công. Không file nào trong đó đụng máy in:

```bash
PRINT_ENGINE=noop ATTENDANCE_DB=/tmp/att_suite.db ATTENDANCE_STAFF_CACHE=/tmp/att_suite_staff.json \
  python3 -m unittest test_attendance_store test_attendance_auth test_attendance_routes test_attendance_sync
```

Kiểm hàng đợi máy in sau mỗi đợt test: `lpstat -o | wc -l` phải ra 0.

### ⚠️ Máy này đang chạy prod, quán đang bán hàng

`print_server.py` **prod đang sống** trên port 5001 (PID kiểm bằng `lsof -nP -iTCP:5001`), phục vụ khách thật, `outbox.db` có gần 1000 đơn thật.

Ba luật tuyệt đối:

1. **Đừng khởi động server trên port 5001, đừng kill tiến trình đó.** Muốn chạy thử thì dùng `SERVER_PORT=5002`.
2. **Mọi lệnh test phải ép DB tạm**, luôn kèm bộ biến môi trường này:
   `PRINT_ENGINE=noop GATEWAY_DB=/tmp/att_suite_outbox.db ATTENDANCE_DB=/tmp/att_suite.db ATTENDANCE_STAFF_CACHE=/tmp/att_suite_staff.json`
   Lý do: `PRINT_ENGINE=noop` chỉ chắn tiến trình test. Test ghi vào `outbox.db` prod thì **worker prod là tiến trình riêng vẫn đọc và in ra máy POS thật** — chuyện này đã xảy ra hai lần, một lần in ra bill ma giữa giờ bán hàng.
3. **Đừng xoá dòng nào trong `outbox.db` prod.** Đó là doanh thu thật.
...
Chạy: `PRINT_ENGINE=noop ATTENDANCE_DB=/tmp/att_suite.db ATTENDANCE_STAFF_CACHE=/tmp/att_suite_staff.json python3 -m unittest test_attendance_store test_attendance_auth test_attendance_routes test_attendance_sync 2>&1 | tail -3`
...
`Code.gs` có **hai** registry: `GET_ROUTES` (dòng 66) và `POST_ROUTES` (dòng 579). `doPost` chỉ tra `POST_ROUTES`, mà worker Python gửi bằng POST. Đặt nhầm vào `GET_ROUTES` là tính năng chết im lặng — worker nhận `unknown_action` mãi mãi.

Thêm vào **`POST_ROUTES`**, cạnh `'swap_order_item'` (khoảng dòng 729), theo đúng dạng handler `function(p)` của các entry sẵn có:
...
Đừng lấy entry `'swap_order_item'` trong `GET_ROUTES` (dòng 254) làm mẫu — nó gọi `_parseBody`, hàm **không tồn tại** trong repo, tức đó là code chết sẵn.
...
- [ ] **Step 4: Chạy thật và xem bằng mắt — CỔNG 5002, KHÔNG PHẢI 5001**

> ⚠️ Port 5001 đang có **print_server prod đang chạy thật** phục vụ quán đang bán hàng.
> Đừng khởi động server trên 5001, đừng `kill` tiến trình đó. Chạy bản test trên 5002 với DB riêng:

```bash
cd print-server && PRINT_ENGINE=noop GATEWAY_SYNC=0 SERVER_PORT=5002 \
  GATEWAY_DB=/tmp/att_test_outbox.db ATTENDANCE_DB=/tmp/att_test.db \
  ATTENDANCE_STAFF_CACHE=/tmp/att_staff.json python3 print_server.py
```

Mở `http://localhost:5002/cham-cong.html`. Kiểm:
...
Chạy từ `print-server/`: `PRINT_ENGINE=noop ATTENDANCE_DB=/tmp/att_suite.db ATTENDANCE_STAFF_CACHE=/tmp/att_suite_staff.json python3 -m unittest test_attendance_store test_attendance_auth test_attendance_routes test_attendance_sync 2>&1 | tail -3`
...
1. **DHCP reservation cho Mac Mini** trên router (giữ `192.168.50.125`) — làm **trước** khi in QR sticker.
...
4. **In QR sticker** trỏ `http://192.168.50.125:5001/cham-cong.html`, kèm dòng chữ "Bật Wifi quán trước khi quét".
