...
Chạy: `PRINT_ENGINE=noop GATEWAY_DB=/tmp/att_suite_outbox.db ATTENDANCE_DB=/tmp/att_suite.db ATTENDANCE_STAFF_CACHE=/tmp/att_suite_staff.json python3 -m unittest discover -p "test_*.py" 2>&1 | tail -5`
...
Chạy từ `print-server/`: `PRINT_ENGINE=noop GATEWAY_DB=/tmp/att_suite_outbox.db ATTENDANCE_DB=/tmp/att_suite.db ATTENDANCE_STAFF_CACHE=/tmp/att_suite_staff.json python3 -m unittest discover -p "test_*.py" 2>&1 | tail -5`
