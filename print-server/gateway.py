    ...
    def unsynced(self):
        ...
        for r in rows:
            if r["op"] != "ingest_order" and r["order_id"] not in synced_creates:
                # Nếu create của order này chưa synced, xem nó có nằm trong hàng chờ unsynced trước r không
                if not any(x["order_id"] == r["order_id"] and x["op"] == "ingest_order" for x in rows if x["seq"] < r["seq"]):
                    continue
            out.append(dict(r))
        return out
