...
```
- Test: mở `https://kaerukaphe.kaerukaphe.workers.dev/kds` → KDS load đơn bình thường (token đã được tiêm).
...
- Application domain: `kaerukaphe.kaerukaphe.workers.dev` path `/dashboard*`, thêm app cho `/kds*`, `/camera*`.
...
GitHub Pages KHÔNG chạy Worker → bản `hibbman23-dpd.github.io/kaerukaphe/dashboard.html` **vẫn hở** dù đã làm hết các bước trên. Chọn 1:
